...
{
    public partial class SwOpAmp : UserControl
    {
        ...
        private void SwOpAmp_Load(object sender, EventArgs e)
        {
        ...
        private void numericTextBoxWithSign1_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                if (SetDefaultVoltage(numericTextBoxWithSign1.DoubleValue) == 1)
                {
       ...
       private void numericTextBoxWithSign2_KeyPress(object sender, KeyPressEventArgs e)
        {
            editing_voltage = true;
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (SetSwitchVoltage(numericTextBoxWithSign2.DoubleValue) == 1)
                {
                    numericTextBoxWithSign2.BackColor = Color.PowderBlue;
                    signal_mainForm(numericTextBoxWithSign2.Parent);
                    editing_voltage = false;
                }
            }
        }
        ...
        public void HamburgBoxAdcValues_UPD_EventMethod(BOX_ADDRESS Addr)
        {
            ...
            if (!editing_voltage)
            {
                ...
                if (perCent_difference > ACCEPTABLE_V_VARIATION && numericTextBoxWithSign1.DoubleValue != 0.0)
